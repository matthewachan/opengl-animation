...
public class OBJLoader {
	...
	// This utility method has 2 primary tasks:
	// (1) Populates indices ArrayList, f, with the vertex position indices (strips away the texture and normal indices)
	// (2) Adds an entry to a TreeMap that maps position indices to texture and normal indices
	static private void extractFace(String line, ArrayList<Vector3f> f, TreeMap<Integer, ArrayList<Integer>> map) {
		...
		// Extract position index, texture index and normal index information
		for (int i = 1; i <= 3; ++i) {
			String[] infoGroup = args[i].split("/");
			vIdx[i - 1] = Integer.parseInt(infoGroup[0]);

			// Case when no texture or normal index information is provided
			if (length > 1) {
		...
		// Add position indices to the indices ArrayList
		f.add(new Vector3f(vIdx[0] - 1, vIdx[1] - 1, vIdx[2] - 1));

		if (length > 1) {
			// Add a new entry to the TreeMap
			for (int i = 0; i < 3; ++i) {
	...
	public static Mesh loadMesh(String fileName) throws Exception {
		...
		// TreeMap will store the map from a position index to an ArrayList of the format [texture index, normal index]
		TreeMap<Integer, ArrayList<Integer>> map = new TreeMap<Integer, ArrayList<Integer>>();

		// Populate ArrayLists and the TreeMap
